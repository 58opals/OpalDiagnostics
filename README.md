...
Host apps own the active minimum level, category filter, and buffer policy. Exact category filters preserve exact matching with `.enabled(_:)` and `.excluded(_:)`. Use `.enabledIncludingSubcategories(_:)` or `.excludedIncludingSubcategories(_:)` when a root category such as `fulcrum` should also match dotted subcategories such as `fulcrum.jsonrpc`, `fulcrum.websocket`, and `fulcrum.reconnect`.
